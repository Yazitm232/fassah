...
export default function Admin() {
  ...
  const handlePin = () => {
    if (pin === ADMIN_PIN) { setUnlocked(true); loadPending(); }
    else { setPinError(true); setTimeout(() => setPinError(false), 1500); }
  };

  const loadPending = async () => {
    setLoading(true);
    const { data } = await supabase.from('spaces').select('*').eq('verified', false).order('created_at', { ascending: false });
    setPending(data ?? []);
    setLoading(false);
  };
    ...
    <div style={{ minHeight: '100vh', background: '#F6FAFE', fontFamily: 'system-ui' }}>
      <div style={{ background: 'linear-gradient(135deg, #1255A0, #2B7FD4)', padding: '20px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          {message && <span style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '6px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: 600 }}>{message}</span>}
          ...
          <button onClick={loadPending} style={{ background: 'rgba(255,255,255,0.15)', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '10px', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>↻ Refresh</button>
        </div>
      </div>
            ...
            {pending.map(space => (
              <div key={space.id} style={{ background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                <div style={{ display: 'flex', gap: '20px' }}>
                  {space.photo_url && (
                    <img src={space.photo_url} alt={space.name} style={{ width: '120px', height: '90px', objectFit: 'cover', borderRadius: '10px', flexShrink: 0 }} />
                  )}
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <div>
                        <span style={{ background: '#EFF6FF', color: '#1255A0', fontSize: '11px', fontWeight: 600, padding: '3px 8px', borderRadius: '20px' }}>{space.type}</span>
                    ...
                    {space.best_times && <p style={{ margin: '4px 0', fontSize: '12px', color: '#666' }}>🕐 {space.best_times}</p>}
                    {space.qibla_notes && <p style={{ margin: '4px 0', fontSize: '12px', color: '#666' }}>🧭 {space.qibla_notes}</p>}
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '16px' }}>
                  ...
                  <button onClick={() => reject(space.id)} style={{
                    padding: '9px 24px', borderRadius: '10px', border: 'none',
                    background: '#ef4444', color: 'white', fontSize: '13px', fontWeight: 700, cursor: 'pointer',
                  }}>✕ Reject</button>
                </div>
              </div>
            ))}
