...
const SpacePopup: React.FC<SpacePopupProps> = ({ space, onClose }) => {
  ...
  useEffect(() => {
    ...
    fetchRecentCheckins(space.id).then(data => {
      ...
      const openCount = recent.filter((c: { is_open: boolean }) => c.is_open).length;
      const busyCount = recent.filter((c: { is_busy: boolean }) => c.is_busy).length;
      const wuduCount = recent.filter((c: { wudu_available: boolean }) => c.wudu_available).length;
      const half = Math.ceil(recent.length / 2);
      const parts: string[] = [];
      if (openCount >= half) parts.push('Usually open');
      if (busyCount >= half) parts.push('can be busy');
      else parts.push('usually quiet');
      if (wuduCount >= half) parts.push('wudu available');
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
              <div>
                <span style={{
                  ...
                  fontWeight: 600, padding: '3px 8px', borderRadius: 20, textTransform: 'uppercase' as const,
                }}>{space.type}</span>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <a
                ...
                style={{
                  ...
                  color: '#1255A0', fontWeight: 600, fontSize: 14, textAlign: 'center' as const,
          ...
          <>
            ...
            <button
              ...
              style={{
                ...
                background: allAnswered ? 'linear-gradient(135deg, #1255A0, #2B7FD4)' : '#e5e7eb',
                color: allAnswered ? '#fff' : '#999',
                fontWeight: 700, fontSize: 15, border: 'none', cursor: allAnswered ? 'pointer' : 'not-allowed',
              }}
            >
              Submit Check-In
            </button>
